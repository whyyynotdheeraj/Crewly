...
export async function POST(request: NextRequest) {
  try {
    ...
    // Attempt to send OTP email if Resend is configured
    let emailSent = false;
    ...
    if (RESEND_API_KEY) {
      try {
        ...
        if (error) {
          console.warn('Resend email error (handled):', error);
          emailStatusMessage = error.message || 'Email delivery failed';
        } else {
          emailSent = true;
          emailStatusMessage = 'Email delivered successfully';
        }
      } catch (mailErr: any) {
        console.warn('Resend send exception (handled):', mailErr);
        emailStatusMessage = mailErr?.message || 'Email service error';
      }
    } else {
      emailStatusMessage = 'Resend API key not configured on server';
    }
    ...
    console.error('Send OTP general failure:', error);
